...
public class ImageUtils {
    ...
    public  @NotNull byte[] imageToBytes(@NotNull BufferedImage image) {
    ...
    public  byte byte4dcolormatch(Color color){
        if (!init) initcolortree();
        int[] target = {color.getRed(), color.getGreen(), color.getBlue()};
        int nearest = (int) tree.findNearest(target);
        return (byte)(nearest < 128 ? nearest : -129 + (nearest - 127));
    }

    private  @NotNull Color c(int r, int g, int b, int a) {
        return new Color(r, g, b, a);
    }

    private  @NotNull Color c(int r, int g, int b) {
